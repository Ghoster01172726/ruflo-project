...
- Engine: Unity **6000.3.23f1** (see `ProjectSettings/ProjectVersion.txt`)
- Gameplay scripts: `Assets/Scripts/*.cs` (e.g. `PlayerController.cs`,
  `PackageSpawner.cs`, `PackageGrabber.cs`, `ShelfSlot.cs`,
  `ConveyorLoopMover.cs`, `HandsMotion.cs`)
- Editor-only tooling: `Assets/Scripts/Editor/*.cs` (scene/prefab builders,
  postprocessors) — must be wrapped in `#if UNITY_EDITOR` or live under an
  `Editor/` folder so they're excluded from player builds
- Scenes: `Assets/Scenes/`, Prefabs: `Assets/Prefabs/`, Materials:
  `Assets/Materials/`, Models: `Assets/Models/`
- `.meta` files are part of Unity's asset identity — always add/edit/rename
  them together with their asset, never orphan one
- No Unity Editor MCP/CLI bridge is configured — agents cannot press Play or
  inspect the live scene. Treat `README.md` (manual Editor setup checklist)
  as the source of truth for what must be wired up by hand in-Editor
  (GameObject hierarchy, component references, trigger colliders)

## Build & Test

- ALWAYS verify C# compiles cleanly (no missing usings/refs) before
  considering a script change done — there is no local `dotnet build`/CLI
  Unity batch-mode configured in this repo, so review the diff carefully
  instead of relying on a build command
- No test framework (`com.unity.test-framework`) is installed yet — if you
  add automated tests, add that package first and put tests under
  `Assets/Tests/`
- ALWAYS keep `.meta` files in sync with the assets they describe
